...
export default function LoginPage() {
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      ...
      // 1. Trigger Apple popup → returns id_token + optional user info
      ...
      // 2. Send to our existing backend route
      ...
      if (res.ok && data.success) {
        // 3. Log user into UserContext — same pattern as OTP flow
        login(data.user)
        router.push('/profile')
      } else {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    // Demo Bypass Logic
    if (email === 'demo@whitemanits.ae' && otp === '1234') {
      // For demo, we might still want to call verify-web or just mock login
      // but the user requested bypass standard OTP verification.
      // We'll proceed with standard request but with the known code.
    }
