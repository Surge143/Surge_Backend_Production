...
declare global {
  interface Window {
    google?: any
    handleGoogleCredentialRegister?: (response: { credential: string }) => void
  }
}
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    const initGSI = () => {
      ...
      window.handleGoogleCredentialRegister = async (response) => {
      ...
      window.google.accounts.id.initialize({
        ...
        callback: window.handleGoogleCredentialRegister,
        ux_mode: 'popup',
      })
        ...
        {step === 'email' ? (
          <>
            <div style={{ marginBottom: '12px', position: 'relative', minHeight: '44px' }}>
              {googleLoading && (
                <div style={styles.socialOverlay}>Signing up with Google…</div>
              )}
              <div ref={googleBtnRef} id="google-signup-btn" style={{ width: '100%' }} />
            </div>
            ...
            >
              <AppleIcon />
              <span>{appleLoading ? 'Redirecting to Apple…' : 'Continue with Apple'}</span>
            </button>
          ...
          <form onSubmit={handleVerifyOtp} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            />
            <button className="btn-primary" style={{ padding: '16px' }} disabled={loading} id="register-verify-btn">
