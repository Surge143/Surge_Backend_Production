  ...
  try {
    ...
    try {
      ...
      if (product.variantId) {
        ...
        if (
          typeof selectedVariant.variantStockQuantity === 'number' &&
          selectedVariant.variantStockQuantity < product.quantity
        ) {
        ...
        if (product.subscriptionId && !selectedSub) {
          return NextResponse.json(
            { error: 'Subscription plan not found for this variant' },
            { status: 400 },
          )
        }
      ...
      try {
        ...
        try {
          ...
          const recurringTotalWithoutWTDiscount =
            priceAfterSubDiscount +
            shippingCharge +
            (priceAfterSubDiscount + shippingCharge) * (taxRate / 100)
          ...
          if (wtDiscount > 0) {
            try {
              ...
              return NextResponse.json(
                { error: 'Failed to apply WT Coins discount' },
                { status: 500 },
              )
            }
          }
            ...
            items: [
              {
                price_data: {
                  ...
                  recurring: {
                    interval: (validatedData.frequency.interval as string).toLowerCase() as
                      | 'day'
                      | 'week'
                      | 'month'
                      | 'year',
