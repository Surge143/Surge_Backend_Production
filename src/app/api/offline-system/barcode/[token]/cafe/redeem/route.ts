...
export async function POST(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    // Serialize concurrent redemptions for the SAME user — without this, two
    // near-simultaneous redeem taps (double-tap on the POS tablet, a retry)
    // can both read the same starting balance before either deducts, letting
    // a customer redeem twice for the price of one. A second request for a
    // DIFFERENT user is completely unaffected and proceeds immediately.
