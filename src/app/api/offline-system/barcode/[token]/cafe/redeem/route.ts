...
export async function POST(req: NextRequest, { params }: { params: { token: string } }) {
    ...
    try {
        ...
        const { stampsRewardRedeemed, referenceId, OrderValue, beansRedeemed } = await req.json() as { stampsRewardRedeemed: boolean, referenceId: string, OrderValue: number, beansRedeemed: boolean }

        const userId = Number(decryptedToken)
        ...
        if (stampsRewardRedeemed) {
            if (!stampRecord) {
                return NextResponse.json({ success: false, message: 'No stamp record found for user' }, { status: 404 })
            }

            if ((stampRecord.stampReward || 0) < 1) {
                return NextResponse.json({ success: false, message: 'Insufficient stamp rewards' }, { status: 400 })
            }
        }
        ...
        if (beansRedeemed) {
            ...
            if (!beanRecord) {
                return NextResponse.json({ success: false, message: 'No bean record found for user' }, { status: 404 })
            }
            ...
            if (remainingToDeduct > 0) {
                return NextResponse.json({ success: false, message: 'Internal error: balance calculation mismatch' }, { status: 500 })
            }
        }
