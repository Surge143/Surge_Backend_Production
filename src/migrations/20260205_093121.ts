...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_app_cart_origin" AS ENUM('app', 'website');
  ...
  CREATE TYPE "public"."enum_web_orders_applied_benefit" AS ENUM('none', 'coupon', 'points');
  ...
  CREATE TABLE "app_orders" (
  	...
  	"selected_slot" varchar NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  CREATE TABLE "user_rewards_redeemed_points_history" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  ...
  CREATE TABLE "user_rewards" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"user_id" integer NOT NULL,
  	"total_earned_points" numeric NOT NULL,
  	"reward_expiry" timestamp(3) with time zone NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  CREATE TABLE "web_orders" (
  	...
  	"shipping_address_emirates" varchar,
  	...
  	"billing_address_emirates" varchar NOT NULL,
  	"billing_address_phone_number" varchar NOT NULL,
  	"payment_status" "enum_web_orders_payment_status" NOT NULL,
  	"delivery_status" "enum_web_orders_delivery_status" DEFAULT 'placed',
  	"applied_benefit" "enum_web_orders_applied_benefit" DEFAULT 'none',
  ...
  ALTER TABLE "user_rewards_redeemed_points_history" ADD CONSTRAINT "user_rewards_redeemed_points_history_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."user_rewards"("id") ON DELETE cascade ON UPDATE no action;
  ...
  ALTER TABLE "web_orders_items" ADD CONSTRAINT "web_orders_items_variant_id_web_products_id_fk" FOREIGN KEY ("variant_id") REFERENCES "public"."web_products"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE INDEX "user_rewards_redeemed_points_history_order_idx" ON "user_rewards_redeemed_points_history" USING btree ("_order");
  CREATE INDEX "user_rewards_redeemed_points_history_parent_id_idx" ON "user_rewards_redeemed_points_history" USING btree ("_parent_id");
  ...
  CREATE INDEX "web_orders_items_variant_idx" ON "web_orders_items" USING btree ("variant_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "user_rewards_redeemed_points_history" CASCADE;
  ...
  DROP TYPE "public"."enum_web_orders_applied_benefit";
