          ...
          <View style={styles.addressCol}>
            ...
            {data.shippingMethod === 'pickup' ? (
              <View style={{ marginTop: 14 }}>
                ...
                <Text style={styles.addrBold}>
                  {data.pickupLocation?.name || 'Surge Coffee'}
                </Text>
                {data.pickupLocation?.address ? (
                  <Text style={styles.addrText}>{data.pickupLocation.address}</Text>
                ) : null}
                {data.pickupLocation?.hours ? (
                  <Text style={styles.addrText}>{data.pickupLocation.hours}</Text>
                ) : null}
              </View>
            ) : data.shipTo ? (
              <View style={{ marginTop: 14 }}>
        ...
        <View style={styles.table}>
          ...
          {data.lineItems.map((item) => (
            <View key={item.id} style={{ ...styles.tableRow, ...styles.tableBodyRow }}>
              ...
              <View style={{ ...styles.td, flex: 5, flexDirection: 'column' }}>
                <Text>{item.name}</Text>
        ...
        <View style={styles.footer}>
          ...
          <View style={styles.footerBottom}>
            <View style={styles.footerLeft}>
              <Text style={{ color: C.dark, marginBottom: 13 }}> {/* Fixed string '13' to number 13 */}
                Contact {"\n"}
                <Text style={{ ...styles.trn, fontWeight: 'normal' }}> {/* Added fontWeight: 'normal' to un-bold the info */}
                  surgeim1@gmail.com {"\n"}
                  +971 4 000 0000 {"\n"}
                  www.surgecoffee.ae
                </Text>
