...
export const changeEmailOtpWeb: PayloadHandler = async (req) => {
    const { payload } = req;

    try {
        if (!req.user) {
            return Response.json(
                { success: false, message: 'You must be logged in to change your email' },
                { status: 401 }
            )
        }
        ...
        try {
            ...
            console.error("Error checking if email exists in changeEmailOtpWeb:", error);
            return Response.json({ success: false, message: 'Failed to verify email availability' }, { status: 500 });
        }
