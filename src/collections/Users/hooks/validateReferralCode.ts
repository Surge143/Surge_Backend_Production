...
export const validateReferralCode: CollectionBeforeChangeHook = async ({
  data,
  operation,
  originalDoc,
  req: { payload },
  ...
  if (!email) {
    console.warn(
      '[validateReferralCode] No email found for validation. Skipping previous order check.',
    )
  } else {
      ...
      if (trulyOldOrder) {
        throw new ValidationError({
          errors: [
            {
              message:
                'Referral codes can only be applied to new accounts with no previous orders.',
    ...
    if (originalDoc?.createdAt) {
      ...
      if (hoursSinceCreation > 24) {
        throw new ValidationError({
          errors: [
            {
              message: 'Referral codes can only be applied within 24 hours of account creation.',
              path: 'referralCodeInput',
            },
          ],
        })
  ...
  const isSelfByEmail =
    email && referrer.email && email.toLowerCase() === referrer.email.toLowerCase()
