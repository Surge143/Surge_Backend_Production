  ...
  setImmediate(async () => {
    ...
    try {
      ...
      if (latestDoc && !(latestDoc as any).isStampsAwarded) {
        ...
        const currentStatus =
          latestDoc.orderType === 'dine-in'
            ? latestDoc.appOrderStatusDine
            : latestDoc.appOrderStatus
        const isCompleted = currentStatus === 'completed'

        if (!isPaid || !isCompleted || !isAccepted) {
          console.log(
            `[afterChange] Order ${doc.id} not yet fully paid, completed, and accepted (Status: ${currentStatus}, Paid: ${isPaid}, Accepted: ${isAccepted}). Skipping stamp accrual for now.`,
          )
          return
        }
        ...
        const orderProductIds: (string | number)[] = (latestDoc.items || [])
        ...
        const eligibleProductsResult = await payload.find({
          ...
          where: {
            and: [{ id: { in: orderProductIds } }, { isStampEligible: { equals: true } }],
          },
          depth: 0,
        ...
        if (stampsEarned > 0) {
          const stampUserId =
            typeof latestDoc.user === 'object' ? latestDoc?.user?.id : latestDoc.user
          ...
          await createStampEarnedNotification(
            payload,
            stampUserId,
            stampsEarned,
            newStampCount,
    ...
    if (previousDoc && previousDoc.slot && previousDoc.slot !== doc.slot) {
      await updateSlotLoad(
        typeof previousDoc.slot === 'object' ? previousDoc.slot.id : previousDoc.slot,
      )
    }
  })
}
